...
% SRL-based resilience specification only
phi.is_base = true;
phi.limits = [0,0];
...
switch numel(varargin)
    ...
    case 1 % string
        ...
        if ischar(st)
            ...
            error('STL_Formula:Bad_argument_type', 'Second argument should be a string.');
        end
        phi = STL_Parse(phi,varargin{:});
                        
    case 2 % string + notbase OR isbase + limits
        if islogical(varargin{1}) % isbase + limits
            if isequal(varargin{1},false) % isbase is false
                error('Tautology must be base case STL-based resilience specification.');
            else 
                phi.st = 'true';
                phi.evalfn = @true_formula;
                phi.type = 'predicate';
                phi.is_base = true;
                assert(all(size(varargin{2}) ==[1,2]), "Wrong format of recoverability and durability limits.");
                phi.limits = varargin{2};
            end
        elseif ischar(varargin{1}) % string + notbase
            if isequal(varargin{2},true) % is base without limits
                error('Need recoverability and durability limits.');
            else
                st = varargin{1};
                if ischar(st)
                    st = regexprep(st,'eventually', 'ev');
                ...
                phi = STL_Parse(phi,varargin{1});
                phi.is_base = false;
            end
        else
            error('STL_Resilience_Formula:Bad_argument_type', 'Second argument should be a string or a tautology.');
        end

    case 3 % base case, string+isbase+limits
        if ~isa(varargin{1},'STL_Formula') && ~ischar(varargin{1})
            error('STL_Formula:Bad_argument_type', 'Second argument should be a string or a formula.');
        elseif ~islogical(varargin{2})
            error('STL_Formula:Bad_argument_type', 'Third argument should be a logical to indicate whether it is a base case STL-based resilience specification.');
        elseif isequal(varargin{2},false)
            error('STL_Formula:Bad_argument_type', 'No need recoverability and durability limits if it is not base case.');
        else
            assert(all(size(varargin{3}) ==[1,2]), "Wrong format of recoverability and durability limits.");
        end
        st = varargin{1};
        ...
        phi = STL_Parse(phi,varargin{1});
        phi.is_base = true;
        phi.limits = varargin{3};

    otherwise
        error('STL_Formula:Bad_argument_number', 'Too much argument.');
%     otherwise
%         st = varargin{1};
%         if(numel(varargin)==1 && ischar(st))
%             st = regexprep(st,'eventually', 'ev');
...
% Hongkai
if ~phi.is_base && isequal(phi.type,'predicate')
    error('A predicate must be a base case.');
end

% Hongkai: for now, no check if any descendant of a base case is also base
% case
