...
dt = 0.0001;
    ...
    function [pos, vel] = get_pos_vel(t)
        ...
        elseif t >= 0 & t < time_tol/4
            ...
            [pos, vel, ~] = tj_from_line([0;0;0], [0;0;20], time_tol/4, t);
        elseif t >= time_tol/4 & t < time_tol/2
            % from [1/4;sqrt(2);sqrt(2)] to [1/2;0;2*sqrt(2)]
            [pos, vel, ~] = tj_from_line([0;0;20], [0;0;20], time_tol/4, t-time_tol/4);
        elseif t >= time_tol/2 & t < time_tol*3/4
            % from [1/2;0;2*sqrt(2)] to [3/4; -sqrt(2); sqrt(2)]
            [pos, vel, ~] = tj_from_line([0;0;20], [0;0;5], time_tol/4, t-time_tol/2);
        else
            % from [3/4; -sqrt(2); sqrt(2)] to [1;0;0]
            [pos, vel, ~] = tj_from_line([0;0;5], [0;0;5], time_tol/4, t-time_tol*3/4);
        end
    end
