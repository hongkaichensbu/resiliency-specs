...
for iter = 1:max_iter
    ...
    for qn = 1:nquad
        ...
        % Run simulation
%         if iter >=800
%                 [tsave, xsave] = ode23s(@(t,s) quadEOM(t, s, qn, controlhandle, trajhandle, params), timeint, x{qn});
%         end
        [tsave, xsave] = ode15s(@(t,s) quadEOM(t, s, qn, controlhandle, trajhandle, params), timeint, x{qn});
        x{qn}    = xsave(end, :)';
...
%     if(t> cstep*250) % increase *50 to *250
%         err = 'Ode45 Unstable';
%         break;
%     end
